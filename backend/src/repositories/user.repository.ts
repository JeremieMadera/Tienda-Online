...
class UserRepository {
  async createUser(email: string, password_hash: string): Promise<PublicUser> {
    const query = "INSERT INTO users (email, password_hash) VALUES ($1, $2) RETURNING id, email, created_at, role";
  ...
  async getUserByEmail(email: string): Promise<StoredUser | undefined> {
    const query = "SELECT id, email, created_at, password_hash, role FROM users WHERE email = $1";
    const result = await pool.query(query, [email]) ;
    return result.rows[0];
  }

  async getUserById(id: number): Promise<PublicUser | undefined> {
    const query = "SELECT id, email, created_at, role FROM users WHERE id = $1";
